...
pub enum Error {
    #[error("Failed to serialize json abi: {0}")]
    SerializeAbi(#[from] serde_json::Error),
}
...
/// structs
pub fn abi_to_solidity(contract_abi: &Contract, mut contract_name: &str) -> Result<String, Error> {
    ...
    let contract_raw_abi: RawAbi = serde_json::from_str(&abi_str)?;

    let structs = InternalStructs::new(contract_raw_abi);
