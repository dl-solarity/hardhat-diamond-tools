...
if [ -z "$PLUGIN_WASM_FILE" ]; then
    PLUGIN_WASM_FILE=../target/wasm32-unknown-unknown/release/diamond_tools_plugin.wasm
fi
...
cp README.md $PLUGIN_OUT_DIR

# copy `readme`, `name`, `description, `version`, `license` from `package.json` to `pkg/package.json`
cat package.json | jq '. | {readme, name, description, version, license}' > $PLUGIN_OUT_DIR/package.json
